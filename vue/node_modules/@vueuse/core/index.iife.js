...
;(function (exports, shared, vueDemi) {
  ...
  function useDark(options = {}) {
    ...
    const store = storageKey == null ? vueDemi.ref("auto") : useStorage(storageKey, "auto", storage, { window, listenToStorageChanges });
  ...
  function useDraggable(target, options = {}) {
    ...
    const start = (e) => {
      ...
      if (vueDemi.unref(options.exact) && e.target !== vueDemi.unref(target))
        return;
      const react = vueDemi.unref(target).getBoundingClientRect();
      const pos = {
    ...
    return __spreadProps$3(__spreadValues$8({}, shared.toRefs(position)), {
      position,
  ...
  function useFocus(options = {}) {
    ...
    const focused = vueDemi.ref(initialValue);
    if (!window)
      return { focused };
    const onFocus = () => {
      focused.value = true;
    ...
    vueDemi.watch(focused, setFocus, { immediate: true, flush: "post" });
  ...
  function useScroll(element, options = {}) {
    ...
    if (element) {
      const onScrollEnd = shared.useDebounceFn(() => {
